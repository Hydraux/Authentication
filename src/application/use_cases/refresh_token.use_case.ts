...
import { InvalidTokenError, RefreshTokenRevokedError } from "src/domain/exceptions/auth.exceptions";
import { InvalidCredentialsError } from "src/domain/exceptions/user.exceptions";
import { JwtPayload } from "src/domain/value_objects/jwt_payload";
import { RefreshTokenRequest } from "../contracts/refresh_token_request";
import { RefreshTokenResponse } from "../contracts/refresh_token_response";
...
export class RefreshTokenUseCase implements IUseCase<RefreshTokenRequest, RefreshTokenResponse> {
  ...
  let payload: { userId: any; token?: string; };
  try {
    payload = await this.tokenGateway.verifyRefreshToken(request.refreshToken);
  } catch (error) {
  ...
  if (!storedRefreshToken || !storedRefreshToken.isValid()) {
    ...
    const newAccessToken = await this.tokenGateway.generateAccessToken(jwtPayload);

    // 5. Generate new refresh token (token rotation for security)
    const newRefreshTokenData = await this.tokenGateway.generateRefreshToken(user.id);
